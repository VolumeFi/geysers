...
interface IUniswapRemove:
    def divestUniPairToToken(pair: address, token: address, amount: uint256, minTokenAmount: uint256, deadline: uint256) -> uint256: payable
...
@internal
def _withdrawToSelf(amount: uint256, withdrawer: address):
...
def _withdraw(amount: uint256, withdrawer: address):
    self._withdrawToSelf(amount, withdrawer)
...
def withdrawToToken(amount: uint256, token: address, minTokenAmount: uint256):
    ...
    self._withdrawToSelf(amount, msg.sender)
    self._safeApprove(_stakingToken, _uniswapRemove, amount)
    retAmount: uint256 = IUniswapRemove(_uniswapRemove).divestUniPairToToken(_stakingToken, token, amount, minTokenAmount, MAX_UINT256, value=fee)
...
def exitToToken(token: address, minTokenAmount: uint256):
    ...
    if msg.value > fee:
        ...
        assert msg.value == fee, "Insufficient fee for UniswapRemove"
    self._withdrawToSelf(_balance, msg.sender)
    self._safeApprove(_stakingToken, _uniswapRemove, _balance)
    retAmount: uint256 = IUniswapRemove(_uniswapRemove).divestUniPairToToken(_stakingToken, token, _balance, minTokenAmount, MAX_UINT256, value=fee)
    if token == ZERO_ADDRESS or token == VETH:
        send(msg.sender, retAmount)
    else:
        self._safeTransfer(token, msg.sender, retAmount)
...
@payable
def __default__():
    assert msg.sender == self.uniswapRemove, "can't receive Eth"
